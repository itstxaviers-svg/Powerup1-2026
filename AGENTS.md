...
- Paired fights after Units 7 and 9 are allocated together from one shuffle into non-overlapping one-third sets. Failed retries keep the same persisted word set and only reshuffle its order. Invalid or deleted IDs in an older allocation are repaired from the current eligible pool without clearing completed fights or other progress.
...
- Battle image metadata is derived from the existing Unit 1–9 vocabulary and the single `battleVocabularyImages` registry. Units 1–3 use approved local files under `assets/Power_Up_1_Units_1_3_Assets`; Units 4–6 and 8–9 use the prepared folders under `assets/battles/checkpoint-07` and `assets/battles/checkpoint-09`. Unit 7 currently has no prepared image folder.
- Only existing enabled `word` items with one unambiguous curriculum answer receive Image → Type. All other eligible Unit words use Audio → Type. Image mappings never reclassify phrases as words, and image availability never changes pool selection. The Unit 1 `crayon` target intentionally uses the `17_crayons` artwork; curriculum `pasta` and British `lorry` remain canonical when using the spaghetti and truck artwork.
- Runtime artwork uses resized WebP variants: vocabulary prompts are capped at 640 px, rewards artwork at 720 px, boss states at 960 px and public background artwork at 1280 px. The current boss and allocated image prompts preload during the battle introduction; lower-page rewards artwork loads lazily. Keep the source PNG/JPEG files recoverable and regenerate the runtime variants with `npm run images:optimise` after approved artwork changes.
